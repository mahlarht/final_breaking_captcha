...
minValidHeight=height*0.03
minValidWidth=width*0.01
maxValidHeight=height*0.95
maxValidWidth=width*0.50
...
for contour in contours:
    ...
    if(w>minValidWidth and h>minValidHeight  and h<maxValidHeight and w<maxValidWidth):
        cv2.rectangle(image, (x, y), (x+w, y+h), (0,0,0),1)

        # select the sample, resize to 10x10 and then vectorize
        roi = thresh[y:y+h, x:x+w]
        cv2.imwrite("temp"+str(x)+".jpg",roi)
        roi= cv2.imread('temp1.jpg')
